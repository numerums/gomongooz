...
//See: http://godoc.org/labix.org/v2/mongo#Query.Select
...
//See: http://godoc.org/labix.org/v2/mongo#Query.Sort
...
//See: http://godoc.org/labix.org/v2/mongo#Query.Limit
...
//See: http://godoc.org/labix.org/v2/mongo#Query.Skip
...
func (self *Query) Exec(result interface{}) error {
	...
	if resultType.Kind() == reflect.Ptr && resultType.Elem().Kind() == reflect.Slice {
		...
		mongoQuery := self.collection.Find(self.query)

		self.extendQuery(mongoQuery)
		...
		//expect all other types - missmatch will panic later or through mongo adapter
	} else {
...
func (self *Query) extendQuery(mongoQuery *mongo.Query) {

	if self.selector != nil {
		mongoQuery.Select(self.selector)
	}

	if len(self.sort) > 0 {
		mongoQuery.Sort(self.sort...)
	}

	if self.limit != 0 {
		mongoQuery.Limit(self.limit)
	}

	if self.skip != 0 {
		mongoQuery.Skip(self.skip)
	}
}
		...
		if structField, ok := document.Elem().Type().FieldByName(populateFieldName); ok {
			...
			if !field.IsNil() {
				...
				case bson.ObjectId:
					...
					if relationError == mongo.ErrNotFound && relationError != nil {
				...
				case []interface{}:
					...
					if relationError == mongo.ErrNotFound || resultSlice.Len() == 0 {
 ...
 * the result reference to the mongo execute function the type
...
func (self *Query) initDocument(model *reflect.Value, document *reflect.Value, collection *mongo.Collection, connection *Connection) {
